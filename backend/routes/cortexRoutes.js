...
async function portfolioContext(){
  const [projects,cards,logistics,assets,cortex]=await Promise.all([
    Project.find().lean(),Card.find({isVisible:{$ne:false}}).lean(),Logistic.find().lean(),Asset.find().lean(),AiCortex.find({enabled:true}).lean()
  ]);
  return {projects:compact(projects),cards:compact(cards),logistics:compact(logistics),assets:compact(assets),cortex:compact(cortex)};
}
function deterministic(message,data){
  const query=message.toLowerCase(); const all=[...data.projects,...data.cards,...data.logistics,...data.assets,...data.cortex];
  const matches=all.filter(item=>JSON.stringify(item).toLowerCase().includes(query)).slice(0,5);
...
async function providerRecommendation(systemPrompt,message,data){
  ...
  const response=await fetch(url.replace(/\/$/,'')+'/chat/completions',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${key}`},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-4o-mini',temperature:0.2,messages:[{role:'system',content:systemPrompt||'Give a concise recommendation using the supplied portfolio context.'},{role:'user',content:`Portfolio context:\n${JSON.stringify(data)}\n\nUser request:\n${message}`}]})});
